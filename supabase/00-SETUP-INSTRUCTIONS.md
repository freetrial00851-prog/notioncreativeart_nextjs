-- ═══════════════════════════════════════════════════════════════════════════
-- notioncreativeart_nextjs — FULL DATABASE SETUP
-- Project: anlsellghialszuuvipw
--
-- Run ONCE in Supabase Dashboard → SQL Editor on the NEW empty project:
--   https://supabase.com/dashboard/project/anlsellghialszuuvipw/sql/new
-- ═══════════════════════════════════════════════════════════════════════════

-- Paste and run each section below IN ORDER, or run this entire file if your
-- SQL editor supports large scripts.

-- ── SECTION 1: Core schema ────────────────────────────────────────────────
-- (Copy contents of schema.sql, storage-setup.sql, site-settings.sql,
--  seed-categories.sql, features-2.sql, cart.sql, newsletter.sql,
--  homepage-layout.sql, catchup-migration.sql, and remaining migrations)

-- After running migrations, run seed-initial-data.sql for starter content.
